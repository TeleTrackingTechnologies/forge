...
class ManagePlugins:
    ...
    def execute(self, args: list) -> None:
        ...
        self._validate_args(parsed_args=parsed_args)
    ...
    @staticmethod
    def _pull_name_from_url(url: str) -> Any:
    ...
    def _clone_plugin_step(self, name, args) -> None:
        ...
        with Halo(
            ...
            try:
                ...
                spinner.succeed(f'Cloned plugin: [{name}]!')

            except PluginManagementFatalException as err:
                spinner.fail(f'Could not clone plugin: [{name}]! ' + str(err))
                raise err from None
    ...
    def _install_dependencies(self) -> None:
        ...
        with Halo(
                text='Installing plugin dependencies...',
                spinner='dots',
                color='blue'
        ) as spinner:
            for plugin_path in self.config_handler.get_plugins():
                ...
                if os.path.exists(req_file):
                    with open(os.devnull, 'wb') as devnull:
                        command = f'{sys.executable} -m pip install -r {req_file}'.split()

                        subprocess.check_call(command,
                                              stdout=devnull,
                                              stderr=subprocess.STDOUT
                                              )
            spinner.succeed('Plugin dependencies installed!')
...
def _write_failure_message(message: str) -> None:
...
def handle_error(message: str) -> None:
    ...
    _write_failure_message(message)
